    ...
    let solve (c) = 
        let rec unify c = 
            match c with
                ...
                | (Type.Var(x), tyt) :: rest -> 
                                let rest = subst x tyt rest in
                                   (x, tyt) :: unify rest

                | (tys, Type.Var(x)) :: rest -> 
                                let rest = subst x tys rest in
                                    (x, tys) :: unify rest
...
let rec check ((env: string -> Type.t), t): Type.t * Constraint.t = 
    match t with   
        ...
        | Abs (x, t) ->
            let new_type_name = freshTypeVar() in
            let new_type = Type.Var(new_type_name) in
            let (rtype, con) = check(
                              (
                               fun y -> if y=x then new_type else env y
                              ),
                              t
                          ) in
            (Type.Arrow(new_type, rtype), con)
        ...
        | App (t1, t2) ->
             let (rtype1, c1) = check(env, t1) in 
             let (rtype2, c2) = check(env, t2) in 
             let ret_type = Type.Var(freshTypeVar()) in
             let newc = [(rtype1, Type.Arrow(rtype2, ret_type))] in

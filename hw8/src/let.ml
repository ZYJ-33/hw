    ...
    let solve (c) = 
        let rec unify c = 
            match c with
                ...
                | (Type.Var(x), tyt) :: rest -> 
                     let n_rest = subst(x, tyt, rest) in
                     let new_map = (x, tyt) in
                     new_map :: unify n_rest

                | (tys, Type.Var(x)) :: rest -> 
                    let n_rest = subst(x,tys,rest) in
                    let new_map = (x, tys) in
                    new_map :: unify n_rest
                
                | (Type.Arrow(s1, s2), Type.Arrow(t1, t2)) :: rest ->
                        let new_cons = [(s1, t1); (s2, t2)] in
                        let c' = List.concat [rest; new_cons] in
                            unify c'
...
let check t : Type.t * Constraint.t = 
    ...
    (*  gamma is a list of (x: string, T: opt) ,
        ...
        (match t with 
            ...
            | Let (x, t1, t2)->
                let (ty, cons) = doit t1 gamma in
                let bind_vars = find_bind gamma in 
                let ty_scheme = Type.generalize (ty, bind_vars) in
                let ty_scheme_opt = Right(ty_scheme) in
                let new_condi = (x, ty_scheme_opt) in
                let new_gamma = new_condi :: gamma in
                let (t2_type, t2_cons) = doit t2 new_gamma in
                let c' = List.concat [cons; t2_cons] in
                    (t2_type, c')
            ...
            | Abs (x, t) ->
                let new_type = freshTypeVar() in
                let new_type_var = Type.Var new_type in
                let new_type_opt = Left new_type_var in
                let new_condi = (x, new_type_opt) in
                let new_gamma = new_condi :: gamma in
                let (ret, cons) = doit t new_gamma in
                (Type.Arrow (new_type_var, ret), cons)
            ...
            | App (t1, t2) ->
                 let (ty1, c1) = doit t1 gamma in
                 let (ty2, c2) = doit t2 gamma in
                 let ret_type = freshTypeVar() in
                 let newc = [(ty1, Type.Arrow(ty2, Type.Var ret_type))] in
                 let c' = List.concat [c1; c2; newc] in
                    (Type.Var ret_type, c')
...
let t1 = Let ("f", Abs("x", Var "x"), App (Var "f", Var "f"))
let _ = pp t1
let _ = typeCheck t1 
